...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, canfocus, isurgent, neverfocus, oldstate, isfullscreen;
	int floatborderpx;
	Client *next;
	Client *snext;
	Monitor *mon;
...
applyrules(Client *c)
{
	...
	/* rule matching */
	c->iscentered = iscentered_default;
	c->isfloating = 0;
    c->canfocus = 1;
	c->tags = 0;
...
runAutostart(void)
{
	system("cd ~/Development/dwm; ./autostart.sh &");
}
